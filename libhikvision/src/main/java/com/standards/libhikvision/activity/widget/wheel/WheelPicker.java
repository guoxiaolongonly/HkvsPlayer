package com.standards.libhikvision.activity.widget.wheel;
...
public class WheelPicker extends View implements IDebug, IWheelPicker, Runnable {
    ...
    public void setSelectedItemData(String itemData) {
        if (TextUtils.isEmpty(itemData)) return;
        int position = -1;
        for (int i = 0; i < mData.size(); i++) {
            if (mData.get(i).equals(itemData)) {
                position = i;
                break;
            }
        }
